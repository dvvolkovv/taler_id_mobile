...
void main() {
  group('DeviceKeysApiClient', () {
    test('registerDeviceKey POSTs cert payload', () async {
      ...
      expect(mock.callLog, ['POST /profile/device-keys']);
    });

    test('getContactKeys returns parsed certs', () async {
      final dio = Dio(BaseOptions(baseUrl: 'https://fake.test'));
      final certInner = {
        'algorithm': 'X25519',
        'devicePk': 'cd' * 32,
        ...
        'GET /profile/contacts/user-2/keys': [
          {
            ...
            'algorithm': 'X25519',
            'validUntil': '2030-01-01T00:00:00.000Z',
            'certificate': '{"algorithm":"X25519","devicePk":"${'cd' * 32}","userId":"user-2","validUntilEpochMs":1800000000000}',
            'signature': 'aa' * 64,
            'revokedAt': null,
            'createdAt': '2026-04-21T00:00:00.000Z',
          }
    ...
    test('revokeDeviceKey POSTs with reason', () async {
      final dio = Dio(BaseOptions(baseUrl: 'https://fake.test'));
      final mock = _MockInterceptor({
        'POST /profile/device-keys/dk-1/revoke': {'id': 'dk-1', 'revokedAt': '2026-04-21T00:00:00.000Z'},
      });
      dio.interceptors.add(mock);
      final client = DeviceKeysApiClient(dio);
