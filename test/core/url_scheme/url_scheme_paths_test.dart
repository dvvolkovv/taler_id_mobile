...
void main() {
  ...
  test('oauth-callback preserves query string', () {
    expect(
        UrlSchemePaths.translate(
            Uri.parse('talerid://oauth-callback?code=abc&state=xyz')),
  ...
  test('unknown talerid host returns null', () {
    expect(
        UrlSchemePaths.translate(Uri.parse('talerid://unknown/path')), isNull);
  });
}
