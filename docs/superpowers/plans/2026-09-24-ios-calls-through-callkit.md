...
- Modify: `lib/core/services/call_state_service.dart`, `test/core/call_state_service_test.dart` (шаг 0)

- [ ] **Step 0: Сначала комната, потом «линия завершена»**

Этот шаг — отдельным коммитом до остальных. `onLineEnded` станет `registry.endConversation`, а конец звонка в CallKit отпускает ручной режим WebRTC. Если комната ещё подключена, WebRTC поднимет аудиоюнит и сам включит сессию — посреди звонка WhatsApp, если линия была на удержании. Экран звонка уже делает это в правильном порядке (задача 15), а `CallStateService` — нет: `endLine` и `endCall` сообщают о конце линии до `room.disconnect()`. Этими путями кладёт трубку плашка (конец у собеседника, `actionCallEnded`, кнопка завершения линии) и `_hangUpAll`.

- `endLine`: внутри `if (line != null)` сначала `await line.room.disconnect()` с тайм-аутом 2 с (тайм-аут и ошибку проглотить, как `_hangUpInner`), потом `_reportLineEnded(name)`.
- `endCall`: отключить комнаты всех линий (параллельно, `Future.wait`, у каждой тайм-аут 2 с), и только потом сообщить о каждой линии; остальное состояние чистить как сейчас (до ожидания — чтобы «звонок закончился» уходил подписчикам сразу).
- Комментарий «почему» — рядом с каждым из двух мест, по строке-две.
- Тесты: в обоих путях хук получает линию уже после `disconnect()` её комнаты (мок комнаты пишет порядок в общий журнал); зависший `disconnect()` не задерживает сообщение дольше тайм-аута.
- Коммит: `fix(звонок): линия сообщает о конце после отключения комнаты`.
