...
class AudioCaptureChannel(engine: FlutterEngine) : EventChannel.StreamHandler {
  companion object {
    ...
    const val TAG = "MeshVoiceCapture"
  }
  ...
  init {
    MethodChannel(engine.dartExecutor.binaryMessenger, METHOD).setMethodCallHandler { call, res ->
      when (call.method) {
        "start" -> {
          try { start(); res.success(null) }
          catch (e: Exception) { res.error("start_failed", e.message, null) }
        }
  ...
  private fun start() {
    ...
    if (rec.state != AudioRecord.STATE_INITIALIZED) {
      Log.e(TAG, "AudioRecord init failed: state=${rec.state} (likely mic permission denied or in use)")
      try { rec.release() } catch (_: Throwable) {}
      throw IllegalStateException("AudioRecord init failed: state=${rec.state}")
    }
