...
///
/// The static body is also served by the backend
/// (`GET /assistant/instructions?locale=…`) with `{{NOW}}` / `{{LANG_NAME}}`
/// placeholders; [composeAssistantPrompt] builds the final prompt from either
/// the server body or the baked-in fallback below — both paths produce
/// identical output for identical bodies.
String assistantSystemPrompt({
  ...
  return composeAssistantPrompt(
    body: locale == 'ru' ? _staticBodyRu : _staticBodyEn,
    locale: locale,
    preferredName: preferredName,
    nameFromProfile: nameFromProfile,
  );
}

/// Builds the full prompt from a server-provided static [body]:
/// name preamble + placeholder substitution (`{{NOW}}`, `{{LANG_NAME}}`).
/// Falls back to identical dynamic values as the baked-in prompt.
String composeAssistantPrompt({
  required String body,
  required String locale,
  String? preferredName,
  bool nameFromProfile = false,
}) {
  ...
  return namePrompt +
      body
          ...
          .replaceAll('{{LANG_NAME}}', _languageDisplayName(locale));
}

const String _staticBodyRu =
    ...
    'ЧАТ В ЗВОНКЕ:\n'
    'Во время звонка у комнаты есть текстовый чат — его видят все участники звонка.\n'
    'Если пользователь говорит "напиши в чат …", "скинь им …", "продиктуй в чат" — вызови send_room_chat с текстом.\n'
    'Это для того, что лучше написать, чем произнести: ссылки, адреса, номера, коды, суммы.\n'
    'Работает только пока идёт звонок; вне звонка инструмент откажет — тогда предложи отправить обычным сообщением.\n'
    'НЕ путай с send_message: тот пишет в диалог мессенджера, а send_room_chat — в чат текущего звонка.\n'
    'Чат комнаты не хранится: вошедший позже написанного раньше не увидит.\n\n'
    'АНАЛИЗ ПЕРЕПИСКИ:\n'
    ...
    'Если аналитик прислал УТОЧНЯЮЩИЕ ВОПРОСЫ — задай их пользователю, и как только получишь ответы, '
    'В ТОМ ЖЕ ходе ОБЯЗАТЕЛЬНО вызови ask_analyst ПОВТОРНО, включив в текст исходную задачу + все уточнения. '
    'КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО говорить "передал/отправил аналитику", если ты не вызвал ask_analyst в этом же ходе.\n'
...
const String _staticBodyEn =
    ...
    'scores) — use web_search instead. If the analyst replies with CLARIFYING QUESTIONS — ask the user, and '
    'once you have the answers you MUST call ask_analyst AGAIN in the same turn, including the original task '
    'plus all clarifications. NEVER say "sent to the analyst" unless you actually called ask_analyst in that turn.\n\n'
    ...
    'MEETINGS: recorded meetings have recaps. "что было на встрече", "какие решения приняли", "кто за что отвечает" — get_meetings for the list, then get_meeting_summary for one. '
    'Transcript lines carry speaker names, but only when the recording made it clear who spoke — an unnamed line means we do not know, so never guess the speaker. '
    'A meeting with recordingFailed=true captured no audio: say the recording failed, do not describe content. '
    'A meeting with recapPending=true is still being transcribed — say it is not ready yet, do not describe content and do not re-run it. '
    'A meeting with no recap at all can be transcribed with transcribe_meeting — it spends balance, so ask first and wait for a yes. '
    'That call returns immediately and the work continues in the background (minutes for a long meeting): tell the user it started and read the result later with get_meeting_summary.\n\n'
    'SESSIONS:\n'
