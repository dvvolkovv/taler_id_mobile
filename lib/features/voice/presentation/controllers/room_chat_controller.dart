...
enum RoomChatMergeMode {
  ...
  /// остаться дыра). Лента ЗАМЕНЯЕТСЯ целиком: сверка по `clientMsgId` с уже
  /// имеющимися записями, которые ПРЕТЕНДУЮТ быть серверным фактом, не
  /// производится — раз в них нельзя быть уверенным, подмешивать их нельзя,
  /// только выбросить. Исключение — свои неподтверждённые сообщения
  /// (`own && msgId == null`): это не устаревший факт, а локальное
  /// намерение, которого в серверном снимке в принципе не может быть, и
  /// `setHistory` их снимает перед очисткой и возвращает обратно, если
  /// страница их не подтвердила — см. `setHistory`'s doc. Не считается
...
class RoomChatController extends ChangeNotifier {
  ...
  ///
  /// Для [RoomChatMergeMode.replace] сверка с уже имеющимися записями не
  /// производится — лента ОЧИЩАЕТСЯ перед вставкой страницы: источник,
  /// вернувший `truncated`, уже дал понять, что старому состоянию верить
  /// нельзя, а «сверить частично и подмешать» и есть тот самый
  /// недобросовестный сплав, ради которого прочитали лишний раз.
  ///
  /// Но это правило — про строки, которые ПРЕТЕНДУЮТ быть серверным фактом
  /// (пришли из более ранней страницы или уже сверились и получили `msgId`).
  /// Свои сообщения, которые ещё в полёте (`own == true`, `msgId == null`)
  /// — это не устаревший серверный факт, а локальное намерение пользователя,
  /// которого в СЕРВЕРНОМ снимке в принципе быть не может, пока сервер его
  /// не подтвердил. Выбросить такую запись вместе с остальными значило бы:
  /// эхо, пришедшее уже после замены, не находит её — `handleIncomingPacket`
  /// не сверяет с несуществующим `clientMsgId` и рисует собственное же
  /// сообщение как чужое, с собственным именем; а отказ POST после замены —
  /// [markFailed] тоже не находит запись, и набранное молча пропадает без
  /// пометки и без возможности повторить. Поэтому такие записи СНАЧАЛА
  /// снимаются с ленты, ПОТОМ лента заменяется страницей, а в конце —
  /// возвращаются обратно те из них, чей `clientMsgId` страница НЕ
  /// подтвердила (если подтвердила — теперь есть более авторитетная копия
  /// с настоящим `msgId`, и возвращать старую значило бы задвоить).
  void setHistory(
    List<RoomChatHistoryMessage> history, {
    RoomChatMergeMode mode = RoomChatMergeMode.prepend,
  }) {
    List<RoomChatMessage>? pendingOwn;
    if (mode == RoomChatMergeMode.replace) {
      pendingOwn = _messages.where((m) => m.own && m.msgId == null).toList();
    ...
    if (pendingOwn != null && pendingOwn.isNotEmpty) {
      final confirmedByPage = history.map((h) => h.clientMsgId).toSet();
      final restored =
          pendingOwn.where((m) => !confirmedByPage.contains(m.clientMsgId));
